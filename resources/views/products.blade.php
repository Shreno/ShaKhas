...
@section('content')
<!-- Title Page -->
	<section class="bg-title-page p-t-50 p-b-40 flex-col-c-m" style="background-image: url({{asset('img/category/'.$ca->img)}});">
		<h2 class="l-text2 t-center" style="text-shadow: 2px 2px 2px black;">
			{{$su->sub_name}}/{{$ca->category_name}}
		</h2>
	</section>
		...
		<div class="container">
			<div class="row">
				...
				<form action="/filter" method="post">
					...
					<div class="leftbar p-r-20 p-r-0-sm">
						...
						<ul class="p-b-54" style="overflow-y:scroll; height:200px;">
							...
							<li class="p-t-4">
								<input type="checkbox" name="brand[]" hidden value="{{$bran->id}}" id="bran{{$bran->id}}"
								<?php
									$bcolor='black';
								...
								?>
								>
								<a class="s-text13 active1" style="cursor: pointer;color:{{$bcolor}}" id="bra{{$bran->id}}">
									{{$bran->brand_name}}
								</a>
							</li>
							<script type="text/javascript">
								$('#bra{{$bran->id}}').click(function(){
									if(this.style.color=='red'){
										this.style.color='black';
										$('#bran{{$bran->id}}').prop('checked', false);
									}else{
										this.style.color='red';
										$('#bran{{$bran->id}}').prop('checked', true);
									}
								});
							</script>
							@endforeach
				...
				<div class="col-sm-6 col-md-8 col-lg-9 p-b-50">
					...
					<div class="row">
						@foreach($list as $lis)
						@if($lis->status=='show'&&$su->id==$lis->sub_id)
